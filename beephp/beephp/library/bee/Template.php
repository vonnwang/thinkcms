...
use bee\exception\TemplateNotFoundException;
use bee\template\TagLib;
...
class Template
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $class         = false !== strpos($type, '\\') ? $type : '\\bee\\template\\driver\\' . ucwords($type);
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        // 添加安全代码及模板引用记录
        $content = '<?php if (!defined(\'BEE_PATH\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . "\n" . $content;
        // 编译存储
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\bee\\template\\taglib\\' . ucwords($tagLib);
        }
        /** @var Taglib $tLib */
            ...
            while ($matches[0]) {
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if ('$bee' == $first) {
                            // 所有以bee.打头的以特殊变量对待 无需模板赋值就可以输出
                            $parseStr = $this->parseBeeVar($vars);
                            ...
                            $parseStr = '\bee\Request::instance()->' . $method . '(' . $params . ')';
                        } else {
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $bee. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parseBeeVar($vars)
    {
        ...
        if ($vars) {
            switch ($type) {
                case 'SERVER':
                    $parseStr = '\\bee\\Request::instance()->server(\'' . $param . '\')';
                    break;
                case 'GET':
                    $parseStr = '\\bee\\Request::instance()->get(\'' . $param . '\')';
                    break;
                case 'POST':
                    $parseStr = '\\bee\\Request::instance()->post(\'' . $param . '\')';
                    break;
                case 'COOKIE':
                    $parseStr = '\\bee\\Cookie::get(\'' . $param . '\')';
                    break;
                case 'SESSION':
                    $parseStr = '\\bee\\Session::get(\'' . $param . '\')';
                    break;
                case 'ENV':
                    $parseStr = '\\bee\\Request::instance()->env(\'' . $param . '\')';
                    break;
                case 'REQUEST':
                    $parseStr = '\\bee\\Request::instance()->request(\'' . $param . '\')';
                    break;
                case 'CONST':
                    $parseStr = strtoupper($param);
                    break;
                case 'LANG':
                    $parseStr = '\\bee\\Lang::get(\'' . $param . '\')';
                    break;
                case 'CONFIG':
                    $parseStr = '\\bee\\Config::get(\'' . $param . '\')';
                    break;
                default:
                    $parseStr = '\'\'';
                    break;
