...
use bee\Config;
use bee\Db;
use bee\db\exception\BindParamException;
use bee\db\exception\DataNotFoundException;
use bee\db\exception\ModelNotFoundException;
use bee\Exception;
use bee\exception\DbException;
use bee\exception\PDOException;
use bee\Loader;
use bee\Model;
use bee\model\Relation;
use bee\model\relation\OneToOne;
use bee\Paginator;
...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\bee\\paginator\\driver\\' . ucwords($config['type']);
    ...
    protected function getCacheKey($value, $options, $bind = [])
    {
        ...
        if (isset($data)) {
            return 'bee:' . $prefix . (is_array($options['table']) ? key($options['table']) : $options['table']) . '|' . $data;
        }
    ...
    public function find($data = null)
    {
        ...
        if (empty($options['fetch_sql']) && !empty($options['cache'])) {
            ...
            if (true === $cache['key'] && !is_null($data) && !is_array($data)) {
                $key = 'bee:' . $this->connection->getConfig('database') . '.' . (is_array($options['table']) ? key($options['table']) : $options['table']) . '|' . $data;
