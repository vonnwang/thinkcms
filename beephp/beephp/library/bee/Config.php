...
class Config
{
    ...
    public static function parse($config, $type = '', $name = '', $range = '')
    {
        ...
        $class = false !== strpos($type, '\\') ?
            $type :
            '\\bee\\config\\driver\\' . ucwords($type);
    ...
    /**
     * 获取和设置配置参数
     * @param $keys string 配置数组索引
     * @param $fileName string 配置文件名
     * @param $module string  应用模块
     * @return mixed
     */
    public static function readByName( $fileName='', $keys='', $module = '')
    {
        if ( empty( $fileName)) {
            return false;
        }

        $file = CONF_PATH.$fileName.EXT;

        if ( ! empty($module)) {
            //读取当前的模块
            if ( $module == 'common') {
                $file = APP_PATH.$module.'/config/'.$fileName.EXT;
            } else {
                $file = APP_PATH.$module.'/config/'.PRODUCE . '/'.$fileName.EXT;
            }
        }
        ...
        return $keys ? $config[$keys] : $config;
    }
}
