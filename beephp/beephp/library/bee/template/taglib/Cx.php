...
class Cx extends Taglib
{
    ...
    public function tagVolist($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \bee\Collection || ' . $name . ' instanceof \bee\Paginator): $' . $key . ' = 0;';
        // 设置了输出数组长度
    ...
    public function tagForeach($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \bee\Collection || ' . $name . ' instanceof \bee\Paginator): ';
        // 设置了输出数组长度
        if (0 != $offset || 'null' != $length) {
            if (!isset($var)) {
                $var = '$_' . uniqid();
    ...
    public function tagEmpty($tag, $content)
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || ((' . $name . ' instanceof \bee\Collection || ' . $name . ' instanceof \bee\Paginator ) && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
        return $parseStr;
    }
