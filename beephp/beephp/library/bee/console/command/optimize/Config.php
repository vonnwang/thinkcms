...
namespace bee\console\command\optimize;

use bee\Config as BeeConfig;
...
class Config extends Command
{
    ...
    protected function buildCacheContent($module)
    {
        ...
        if ($module) {
            // 加载模块配置
            $config = BeeConfig::load(CONF_PATH . $module . 'config' . CONF_EXT);

            // 读取数据库配置文件
            $filename = CONF_PATH . $module . 'database' . CONF_EXT;
            BeeConfig::load($filename, 'database');

            // 加载应用状态配置
            if ($config['app_status']) {
                $config = BeeConfig::load(CONF_PATH . $module . $config['app_status'] . CONF_EXT);
            }
            // 读取扩展配置文件
            if (is_dir(CONF_PATH . $module . 'extra')) {
                ...
                foreach ($files as $file) {
                    if (strpos($file, CONF_EXT)) {
                        ...
                        BeeConfig::load($filename, pathinfo($file, PATHINFO_FILENAME));
                    }
                }
            }
        }
        ...
        if (is_file(CONF_PATH . $module . 'tags' . EXT)) {
            $content .= '\bee\Hook::import(' . (var_export(include CONF_PATH . $module . 'tags' . EXT, true)) . ');' . PHP_EOL;
        }
        ...
        $content .= '\bee\Config::set(' . var_export(BeeConfig::get(), true) . ');';
        return $content;
    }
}
